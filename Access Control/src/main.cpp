...
#include <ElegantOTA.h> // The "No-More-Laptop" Fix
#include <WiFiUdp.h>
#include <NTPClient.h>
#include <EEPROM.h>

// --- VERSION ---
const char* app_version = "v1.0.3 - WEB OTA";
...
void handleWebClient() {
  ...
  if (!isAuthenticated) {
    ...
    client.println("<div class='card'><h2>🏠 Panel admin</h2><p>Wersja: " + String(app_version) + "</p>");
    client.println("<p>Drzwi: " + String(doorOpen ? "OTWARTE" : "ZAMKNIETE") + "</p>");
...
void loop() {
  ArduinoOTA.handle();
  ElegantOTA.loop(); // Handle web uploads

  // Learning Mode Blinking
  ...
  if (digitalRead(BUTTON_PIN) == LOW) {
    ...
    if(digitalRead(BUTTON_PIN) == LOW) {
      openDoor("Przycisk");
      while(digitalRead(BUTTON_PIN) == LOW) { ArduinoOTA.handle(); ElegantOTA.loop(); } 
    }
  }
