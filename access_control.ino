...
void renderSystemUI() { // [cite: 50]
  ...
  if (provisioningMode || isOfflineStandby) { // [cite: 51]
    ...
    int shackleOffset = (globalAnimFrame > 4) ? 5 : globalAnimFrame; // [cite: 54]
  ...
  if (WiFi.status() == WL_CONNECTED) { // [cite: 63]
    display.print(app_version); // Unique OLED verification tag outputs version text string
...
void sendExternalTelemetry(String logData) { // [cite: 68]
  if (blockTelemetry || !telemetryEnabled || WiFi.status() != WL_CONNECTED) return; // [cite: 68]
...
void handleWebServer() { // [cite: 94]
  ...
  if (reqHeader.indexOf("GET /api/forgot_password") != -1) { // [cite: 96]
    ...
    delay(1); client.stop(); blockTelemetry = false; return; // [cite: 98]
  } // [cite: 99]

  if (failedLoginAttempts >= 5 && millis() < lockoutEndTime) { // [cite: 99]
    client.println("HTTP/1.1 403 Forbidden\r\nContent-Type: text/plain\r\nConnection: close\r\n\r\n[ALERT] LOCKOUT ACTIVE."); // [cite: 99]
    delay(1); client.stop(); blockTelemetry = false; return; // [cite: 100]
  } // [cite: 100]
...
HEADER_COMPLETE: // [cite: 115]
    ...
    if (contentLength <= 0) { // [cite: 117]
        ...
        blockTelemetry = false; // [cite: 118]
        return; // [cite: 118]
    } // [cite: 119]
    addLog("OTA START. SIZE: " + String(contentLength)); // [cite: 119]
    
    InternalStorage.close(); // [cite: 119]
  ...
  if (reqHeader.indexOf("GET /api/data") != -1) { // [cite: 127]
    ...
    delay(1); client.stop(); blockTelemetry = false; return; // [cite: 136]
  } // [cite: 136]
  ...
  if (isAuthenticated) { // [cite: 136]
    if (reqHeader.indexOf("/api/unlock") != -1) { // [cite: 136]
      ...
      client.println("HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\n\r\nOK"); // [cite: 144]
      delay(1); client.stop(); blockTelemetry = false; return; // [cite: 144]
    }  // [cite: 145]
      ...
      client.println("HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\n\r\nOK"); // [cite: 148]
      delay(1); client.stop(); blockTelemetry = false; return; // [cite: 149]
    } // [cite: 149]
      ...
      client.println("HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\n\r\nOK"); // [cite: 159]
      delay(1); client.stop(); blockTelemetry = false; return; // [cite: 160]
    } // [cite: 160]
      ...
      client.println("HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\n\r\nOK"); // [cite: 171]
      delay(1); client.stop(); blockTelemetry = false; return; // [cite: 171]
    } // [cite: 172]
      ...
      client.println("HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\n\r\nOK"); // [cite: 177]
      delay(1); client.stop(); blockTelemetry = false; return; // [cite: 177]
    } // [cite: 177]
  } // [cite: 177]
  blockTelemetry = false; // [cite: 177]
} // [cite: 177]
...
void executeCloudSynchronization() { // [cite: 189]
  ...
  bool serverUnlockSignal = (payloadResponse.indexOf("\"unlock\":true") != -1); // [cite: 195]
  bool serverLearnSignal  = (payloadResponse.indexOf("\"learn\":true") != -1); // [cite: 196]
...
void transmitCardPayloadToCloud(String uidStr, byte* rawUid, bool runRegister) { // [cite: 200]
  ...
  while ((httpPost.available() || httpPost.connected()) && millis() < deadline) { // [cite: 205]
    if (httpPost.available()) { // [cite: 205] 
      // 🌟 FIXED LOGIC BUG: Maps data chunk reader cleanly to current httpPost instance pointer
...
void setup() { // [cite: 206]
  ...
  digitalWrite(LED_RED, LOW); // [cite: 210]
  SPI.begin(); // [cite: 210] 
  RTC.begin(); // [cite: 210] 
...
void loop() { // [cite: 222]
  ...
  if (learningMode) { // [cite: 235]
    ...
    if (failedLoginAttempts >= 5 && millis() < lockoutEndTime) { // [cite: 237]
      ...
      if (isOfflineStandby) { // [cite: 238] 
        ...
        digitalWrite(LED_RED, LOW); // [cite: 239]
      } // [cite: 240]
      digitalWrite(LED_GREEN, LOW); // [cite: 240]
    } // [cite: 240]
  } // [cite: 240]

  // 🌟 ANTENNA CHARGING STABILIZATION GATE (Fixes keychain read delays)
  if (!rfidResetPending && !doorOpen && (failedLoginAttempts < 5 || millis() > lockoutEndTime) && rfid.PICC_IsNewCardPresent()) { // [cite: 240]
    delay(20); // 20ms hardware stabilization window allows smaller fob antennas to finish power induction charging
    if (rfid.PICC_ReadCardSerial()) { // [cite: 240]
      ...
      if (learningMode) { // [cite: 243]
        ...
        if (valid) { // [cite: 249] 
          ...
          addLog("Odmowa: Nieznany [" + uidStr + "]"); // [cite: 251]
  ...
  if (doorOpen && millis() > accessEndTime) { // [cite: 262]
    ...
    digitalWrite(LED_RED, LOW); // [cite: 263]
  } // [cite: 263]
}