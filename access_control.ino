...
// 🛡️ TELEMETRY SHIELD: Prevents network thread collisions during active inbound streams
...
void handleWebServer() { // 
  ...
  blockTelemetry = true; // Activate safety shield gate
  String reqHeader = ""; // 
  ...
  if (learningMode) { // 
    ...
    if (failedLoginAttempts >= 5 && millis() < lockoutEndTime) { // 
      ...
      if (isOfflineStandby) { // 
        ...
        digitalWrite(LED_RED, LOW); // Dark standby
      } //
      digitalWrite(LED_GREEN, LOW); // 
    } //
  } //
