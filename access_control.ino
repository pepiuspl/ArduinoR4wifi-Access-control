...
// 🛡️ CRITICAL FLAG: Stops client socket calls from running during incoming request streams
...
void renderSystemUI() { // [cite: 50]
  ...
  if (provisioningMode || isOfflineStandby) { // [cite: 51]
    ...
    display.drawFastVLine(25, 41, 5, SH110X_BLACK); // [cite: 43]
    display.drawCircleHelper(25, 32, 7, 1|2, SH110X_WHITE); // [cite: 43]
    display.drawFastVLine(18, 32, 4, SH110X_WHITE); // [cite: 43]
...
void sendExternalTelemetry(String logData) { // [cite: 68]
  if (blockTelemetry || !telemetryEnabled || WiFi.status() != WL_CONNECTED) return; // Prevent network threads crashing the S3 module
...
HEADER_COMPLETE:
  ...
  if (isAuthenticated) { // [cite: 136]
    if (reqHeader.indexOf("/api/unlock") != -1) { // [cite: 136]
      if (!doorOpen) openDoor("Panel API"); // Prevent beep loop spamming
      client.println("HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\n\r\nOK"); // [cite: 137]
...
void executeCloudSynchronization() { // [cite: 189]
  ...
  // 🌟 READ POLL IMPULSE ATTRIBUTES DIRECTLY FROM BACKEND RESPONSES
...
void setup() { // [cite: 206]
  ...
  if (WiFi.status() == WL_CONNECTED) { // [cite: 218]
    ...
    delay(140); // [cite: 220] 
    tone(BUZZER_PIN, 1600, 300); // [cite: 260]
  } else { // [cite: 260]
    isOfflineStandby = true; // [cite: 260] 
    forceHardwareRFIDReset(); // [cite: 260] 
    displayProvisioningInstructions("ERR: CONN TIMEOUT"); // [cite: 260]
    WiFi.disconnect(); // [cite: 260] 
    delay(500); // [cite: 260] 
    WiFi.beginAP("ZAMEK_SETUP"); // [cite: 260]
  ...
  if (learningMode) { // [cite: 235]
    ...
    if (failedLoginAttempts >= 5 && millis() < lockoutEndTime) { // [cite: 237]
      ...
      if (isOfflineStandby) { // [cite: 238] 
        ...
        digitalWrite(LED_RED, LOW); // Standby dark on normal link state
      } // [cite: 240]
